...
- **Venue routing (multi-venue):** the inbound webhook resolves which venue owns
  ...
  Unknown accounts currently fall back to `"main"`. That fallback is a production
  blocker; production must reject unknown receiving accounts before the Phase 6
  gate closes. Saving a store's number registers its route (`registerChannelAccount`).
...
- Authenticate ingress before parsing. A provider-specific fast acknowledgement
  must not create a side effect for an untrusted event; durable event capture/queue
  processing is the target design.
...
<!-- PRODUCTION_GO_LIVE_DOMAIN: omnichannel-agent -->
