-- Centralize an order's collected principal after refunds.
-- Refunds can reopen a bill; all QR, split, floor and walkout reads must agree.
          ...
          WHEN payment.kind = 'refund' THEN -COALESCE(
            (
              ...
              WHERE adjustment.refund_id = payment.id
                AND adjustment.component IN ('principal', 'ar')
            ),
            0
          )
